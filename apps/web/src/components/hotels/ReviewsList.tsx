...
interface ReviewsListProps {
  hotelId: string;
  initialReviews: HotelReviewWithUser[];
...
export function ReviewsList({
  ...
  const [reviews, setReviews] = useState<HotelReviewWithUser[]>(initialReviews);
    ...
    <div>
      ...
      {isAuthenticated && !success && !showForm && (
        <button
          ...
          className="mb-5 border border-orange-200 text-orange-700 bg-orange-50 hover:bg-orange-100 px-5 py-2.5 rounded-xl text-sm font-medium transition"
        >
          ✍️ Write a review
        </button>
      )}
      {!isAuthenticated && (
        <a
          href="/auth/login"
          className="inline-block mb-5 border border-gray-200 text-gray-600 px-5 py-2.5 rounded-xl text-sm font-medium hover:bg-gray-50 transition"
        >
          Sign in to write a review
        </a>
      )}
      {success && (
        <div className="mb-5 bg-green-50 border border-green-200 rounded-xl px-5 py-3 text-sm text-green-700">
        ...
        >
                ...
                >
                  <svg
                    className={`w-7 h-7 transition-colors ${
                      s <= (hoverRating || rating) ? "text-amber-400" : "text-gray-200"
                    }`}
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
          ...
          <div className="flex gap-3">
            <button
              type="submit"
              disabled={loading}
              className="bg-orange-600 text-white px-5 py-2.5 rounded-xl text-sm font-semibold hover:bg-orange-700 transition disabled:opacity-50"
            >
            ...
            <button
              ...
              className="text-gray-500 px-5 py-2.5 rounded-xl text-sm hover:bg-gray-50 transition"
            >
              Cancel
            </button>
          </div>
